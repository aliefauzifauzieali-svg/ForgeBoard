...
export function KanbanColumn({
  ...
  return (
    <section
      ...
      data-column={status}
      className="flex w-full animate-fade-up flex-col rounded-xl border border-zinc-200 bg-zinc-50/80 p-3 transition-[border-color,box-shadow] duration-200 ease-spring dark:border-white/[0.06] dark:bg-white/[0.02] lg:w-72 lg:shrink-0 lg:snap-start xl:w-80"
    >
...
export function KanbanBoard({
  ...
  return (
    <div
      ...
      role="region"
      aria-label="Quadro Kanban. Arraste tarefas entre as colunas (mouse ou toque longo) ou use os botões Mover de cada cartão."
    >
      {COLUMNS.map((status) => (
        <KanbanColumn
          key={status}
