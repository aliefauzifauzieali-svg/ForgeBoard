...
export function ShortcutsDialog(): React.JSX.Element {
  ...
  return (
    <Modal
      ...
      description={
        isTouch
          ? 'Dicas para este dispositivo.'
          : 'Os atalhos não funcionam enquanto você digita em campos de texto.'
      }
      onClose={() => setOpen(false)}
    >
      {isTouch ? (
        <p className="text-sm text-zinc-600 dark:text-zinc-400">
          Atalhos de teclado precisam de teclado físico. Neste dispositivo, toque
          nos botões da interface — todas as ações estão visíveis em menus,
          barras e cartões.
        </p>
      ) : (
        <>
